...
class ChecklistIT extends CI_Controller
{
	var $bulan = array(
		"Januari"   => "1",
		"Februari"  => "2",
	...
	public function pc_list($bulan = '', $tahun = '', $shift = '', $id = '')
	{
		if ($bulan == 'nope' && $tahun == 'nope' && $shift == 'nope') {
			$data = $this->DataPC->getIT('', '', '', $id);
		} else if ($bulan != '' && $tahun != '' && $shift != '') {
			$data = $this->DataPC->getIT($bulan, $tahun, $shift);
		} else {
			$data = $this->DataPC->getIT();
		}
	...
	public function editClosed($bagian, $id)
	{
		if ($bagian == "pc") {
			$this->DataPC->editClosed($id);
		}
	}
}
